    ...
    $(document).on('ready', () => {
        ...
        calendar = new FullCalendar.Calendar(calendarEl[0], {
            ...
            eventContent: (info) => {
                ...
                // Verifica se non ci sono posti disponibili
                const noAvailability = info.event.extendedProps.available === 0;
                
                // Formato leggibile della data e ora per il log
                const eventStart = new Date(info.event.start);
                const formattedDate = eventStart.toLocaleString('it-IT', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit',
                    hour: '2-digit',
                ...
                // Crea la classe principale con condizione per aggiungere fc-event-main quando non ci sono posti
                const mainClass = noAvailability 
                    ? 'fc-event-main-frame not-available fc-event-main' 
                    : 'fc-event-main-frame';  
                       
                let str = '<div class="' + mainClass + '">';
                str += info.timeText && ('<div class="fc-event-time"><span class="time-prefix">Ore: </span>' + info.timeText + '</div>');
                str += '<div class="fc-event-title-container">';
                str += '<div class="fc-event-title fc-sticky">';
                str += info.event.title || '&nbsp;';

                // Aggiungiamo i posti disponibili
                if (info.event.extendedProps.available !== undefined) {
                    str += ' <span class="available-spots">(' + info.event.extendedProps.available + ' posti)</span>';
                }
            ...
            eventClick: info => {
                ...
                // Se l'evento non ha disponibilità (ha la classe not-available), non fare nulla
                if (info.event.extendedProps.available === 0) {
                    return;
                }
        ...
        if (typeof calendar === 'object' && typeof calendar.refetchEvents === 'function' && locationSelectEl.length === 1 && qtySelectEl.length === 1) {
            ...
            let submitButton = locationSelectEl.closest('form').find('.elementor-field-type-submit button').trigger('click');
