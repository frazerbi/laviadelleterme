...
function coupon_sent_to_admin_order_status($order) {
    global $wpdb;
    ...
    // 1. $userId = Retrieve authenticated user's id from SESSION
    // 2. $orderUserId = Retrieve order user's id
    // if (($userId !== $orderUserId)  || [!isAdmin()] is not admin user so he cannot do anything ) {
    //     return;
    // }
    ...
    if ($order_status != 'coupon-sent') {
        ...
        $heading ='Errorr in sending order info';
        ...
        $mailer->send('francesco.zerbinato@gmail.com',$heading ,$html_message1, HTML_EMAIL_HEADERS );
        return;
    }
    ...
    $billing_email  = 'francesco.zerbinato@gmail.com';
    $billing_email1  = 'direzione@laviadelleterme.com';
    ...
    foreach ($order_data->get_items() as $lineItem) {
        ...
        if ($variation_id == 0) {
            ...
            // https://www.html.it/articoli/wordpress-eseguire-query-avanzate-con-wpdb/
        ...
        $subject = "$product_name, ecco il tuo biglietto - Ordine: $order_id";
        ...
        // // Titles list
        // $titleList = array(
        //     '164' => 'Biglietto di Ingresso Lunedì-Domenica 4h',
        //     '329' => 'Biglietto di Ingresso Lunedì-Venerdì 4h'
        // );
        // $title = !empty($titleList[$product_id]) ? $titleList[$product_id] : '--- Titolo di default ---';
        ...
        // Send the email using wordpress mail function: biglietti@laviadelleterme.it
        $mailer->send('biglietti@laviadelleterme.it', $subject, $html_message, HTML_EMAIL_HEADERS, $attachments);
...
add_action('woocommerce_order_action_send_coupons_to_admin', 'coupon_sent_to_admin_order_status');
